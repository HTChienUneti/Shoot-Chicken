...
public class EnemyDamageReceiver : DamageReceiver
{
    ...
    {
        this.hpMax = this.enemyCtrl.EnemySO.maxHp;
        base.ResetValue();
    }
    ...
    {
        List<ItemDropRate> itemDrop = this.enemyCtrl.EnemySO.itemDrop;
        ...
        foreach (ItemDropRate drop in itemDrop)
        {
            ...
            Transform item = ItemSpawner.Instance.Spawn(drop.itemDrop.itemProfile.id.ToString(), transform.position, transform.rotation);
            if (item == null) return;
